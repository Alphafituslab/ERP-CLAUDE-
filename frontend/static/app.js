  ...
  async function tratarAcao(acao, alvo) {
    switch (acao) {
      ...
      case "alternar-menu-desktop": {
        state.menuLateralOculto = !state.menuLateralOculto;
        localStorage.setItem("alphafitus_menu_oculto", state.menuLateralOculto ? "1" : "0");
        document.querySelector(".layout").classList.toggle("menu-lateral-oculto", state.menuLateralOculto);
        alvo.textContent = state.menuLateralOculto ? "▶" : "◀";
        return;
      }
